/**
 * SECURITY: MapLibre popups render raw HTML (not React), so GeoJSON properties
 * MUST be DOMPurify-sanitized here to prevent Stored XSS from map data.
 */
import { sanitize, sanitizeGeoJSONProperties } from '@/lib/security/sanitize';

// Both values and keys are sanitized (a malicious key name is also an XSS vector).
