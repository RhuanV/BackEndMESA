...
def init_commands():
    """Read the Airflow initialization script and extract which DAGs are unpaused and triggered automatically."""
    ...
    with open(INIT_SCRIPT_PATH, "r", encoding="utf-8") as f:
        content = f.read()
        # Extract DAGs from the 'airflow dags unpause [dag_id]' command, ignoring flags like '-e'
        unpause_matches = re.findall(r"airflow\s+dags\s+unpause\s+(?:-[^\s]+\s+)*([a-zA-Z0-9_-]+)", content)
        unpaused_dags.update(unpause_matches)

        # Extract DAGs from the 'airflow dags trigger [dag_id]' command
...
def test_initialization_dags_are_active_and_triggered(init_commands):
    """Verify that the configured DAGs are unpaused and had executions triggered automatically."""
