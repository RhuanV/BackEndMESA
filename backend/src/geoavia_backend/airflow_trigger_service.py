"""Service that triggers Airflow DAGs and writes the audit log (Sprint 4 HU-23).

Uses the Airflow stable REST API (`POST /api/v1/dags/{dag_id}/dagRuns`) with
basic auth `admin:admin` — same credentials used in start.sh.

stdlib `urllib.request` avoids adding `requests` to backend/requirements.txt.
"""
...
from geoavia_backend.airflow_trigger_repository import AirflowTriggerRepository
