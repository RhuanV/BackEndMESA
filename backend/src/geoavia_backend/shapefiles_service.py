"""Business logic for HU-31 — receives an uploaded zipped shapefile, extracts,
reads with geopandas, reprojects to SIRGAS 2000 (EPSG:4674), and persists
features in `mesa_a.user_uploaded_features`.

Accepted format: a single ZIP containing at least `.shp` + `.dbf` + `.shx`
(`.prj` strongly recommended — without it source SRID is unknown and the
input is assumed to already be in 4674).
"""
